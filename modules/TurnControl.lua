...
local PRIMARY_URL_STATS = "http://dunerank.servehttp.com:8080"
local PRIMARY_URL_STATS_DEV = "http://localhost:8080"
local IN_DEVELOPMENT = false

if IN_DEVELOPMENT then
    PRIMARY_URL_STATS = PRIMARY_URL_STATS_DEV
end
...
function TurnControl.onLoad(state)
    if state.settings then
        if state.TurnControl then
            ...
            TurnControl.gameId = state.TurnControl.gameId
            broadcastToAll("Game reload. Game ID: " .. tostring(TurnControl.gameId), {1, 1, 1})
...
function TurnControl.start()
    ...
    if TurnControl.gameId == nil then
        TurnControl._sendInitRoundDetails()
    end
...
function TurnControl.endOfPhase(haltAfter)
    ...
    if phase == "combatEnd" then
        TurnControl._sendRoundDataSnapshot()
    end
...
---
function TurnControl._sendInitRoundDetails()
    Helper.onceStabilized().doAfter(function ()
        local playersList = {}
        for _, color in ipairs(PlayBoard.getActivePlayBoardColors()) do
            ...
            if player then
                if IN_DEVELOPMENT then
                -- Fake Steam id for local testing.
                ...
                steamId = player.steam_id or "unknown"
            end
        ...
        WebRequest.custom(
            PRIMARY_URL_STATS .. "/api/games/start",
            "POST",
            true,
            JSON.encode({ players = playersList }),
            {
            ...
            function(response)
                if not response.is_error then
                    ...
                    if data and data.id then
                        TurnControl.gameId = tostring(data.id)
                        broadcastToAll("Game started. Game ID: " .. tostring(TurnControl.gameId), {1, 1, 1})
                    else
                        Helper.dump("[ERROR] Game start response missing id field")
                    end
                else
                    Helper.dump("[ERROR] Failed to start game: " .. response.error)
                end
            end
        )
...
---
function TurnControl._sendRoundDataSnapshot()
    Helper.onceStabilized().doAfter(function ()
        local roundNumber = TurnControl.getCurrentRound()
        local playersData = {}

        local getNamesFromList = function(cards)
            return Helper.mapValues(cards or {}, function(c)
                return (c.getName and c.getName()) or (c.nickname or "(unnamed)")
            end)
        end

        for _, color in ipairs(PlayBoard.getActivePlayBoardColors()) do
            local content = PlayBoard.getContent(color) or {}
            local player = Helper.findPlayerByColor(color)
            local steamId = "unknown"
            if player then
                if IN_DEVELOPMENT then
            ...
            local steamName = player and player.steam_name or "unknown"

            -- DRAW PILE
            local drawZone = content.drawDeckZone
            local drawDeckOrCard = drawZone and Helper.getDeckOrCard(drawZone)
            local drawCards = Helper.getCards(drawDeckOrCard)
            local drawNames = getNamesFromList(drawCards)

            -- DISCARD
            local discardZone = content.discardZone
            local discardCard = discardZone and Helper.getDeckOrCard(discardZone)
            local discardCards = Helper.getCards(discardCard)
            local discardNames = getNamesFromList(discardCards)

            -- TRASH
            local trashGUID = PlayBoard.trashGuidPerColor and PlayBoard.trashGuidPerColor[color]
            local trashBag = trashGUID and getObjectFromGUID(trashGUID)
            local trashCards = {}
            local trashNames = {}
            if trashBag then
                trashCards = Helper.getCardsAndDecksFromBag(trashBag)
                trashNames = Helper.mapValues(trashCards, function(entry)
                    return entry.nickname or entry.name or "(unnamed)"
                end)
            end

            -- AGENT
            local agentZone = PlayBoard.agentParkZoneByColor and getObjectFromGUID(PlayBoard.agentParkZoneByColor[color])
            local agentCards = Helper.getAllCardsInZone(agentZone)
            local agentNames = getNamesFromList(agentCards)

            -- REVEAL
            local revealZone = PlayBoard.revealParkZoneByColor and getObjectFromGUID(PlayBoard.revealParkZoneByColor[color])
            local revealCards = Helper.getAllCardsInZone(revealZone)
            local revealNames = getNamesFromList(revealCards)

            local filteredHandCards = Helper.filterCardsByTag(Player[color].getHandObjects(), "imperium")
            local inHand  = getNamesFromList(filteredHandCards)

            -- LOG SNAPSHOT LOCALLY
            --Helper.dump("[" .. color .. "] End of Turn Zones", {
            --    drawPile = drawNames,
            --    discardPile = discardNames,
            --    trashedCards = trashNames,
            --    agentCards = agentNames,
            --    revealedCards = revealNames,
            --    inHand  = inHand ,
            --})
            ...
            table.insert(playersData, {
                ...
                color = color,
                agentCards = agentNames,
                revealCards = revealNames,
                discardPile = discardNames,
                trashedCards = trashNames,
                acquiredCards = {}, -- can fill in later if needed
                drawPile = drawNames,
                inHand  = inHand
            })
        end
        ...
        if TurnControl.gameId then
            local url = PRIMARY_URL_STATS .. "/api/games/" .. TurnControl.gameId .. "/rounds"
            WebRequest.custom(
                url,
                "POST",
                true,
                ...
                function(result)
                    if result.is_error then
                        Helper.dump("[ERROR] Failed to send round data: " .. result.error)
                    else
                        Helper.dump("[INFO] Round data sent successfully.")
                    end
                end
            )
        else
            Helper.dump("[WARN] No gameId set in TurnControl")
        end
    end)
end
