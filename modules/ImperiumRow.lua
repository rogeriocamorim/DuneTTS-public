...
function ImperiumRow.setUp(settings)
    ...
    Deck.generateImperiumDeck(ImperiumRow.deckZone, settings.useContracts, settings.riseOfIx, settings.immortality, settings.legacy, settings.community).doAfter(function (deck)
...
function ImperiumRow.acquireReservedImperiumCard(color)
    ...
    if cardOrDeck then
        ...
        if card then
            -- Check if player has enough persuasion to acquire the reserved card
            ...
            if playerPersuasion >= cardCost then
                ...
                Dialog.showYesOrNoDialog(color, I18N("confirmAcquireWithoutPersuasion", {
                    ...
                    cost = cardCost,
                    persuasion = playerPersuasion
                }), Helper.createContinuation("ImperiumRow.acquireReservedImperiumCard"), function(confirmed)
                    if confirmed then
...
function ImperiumRow.acquireImperiumCard(indexInRow, color)
    ...
    local card = Helper.getCard(zone)
    if card then
        -- Check if player has enough persuasion to acquire the card
        local cardInfo = ImperiumCard._resolveCard(card)
        local cardCost = cardInfo.cost or 0
        local playerPersuasion = PlayBoard.getResource(color, "persuasion"):get()
        ...
        if playerPersuasion >= cardCost then
            ...
            Dialog.showYesOrNoDialog(color, I18N("confirmAcquireWithoutPersuasion", {
                ...
                cost = cardCost,
                persuasion = playerPersuasion
            }), Helper.createContinuation("ImperiumRow.acquireImperiumCard"), function(confirmed)
                if confirmed then
                    printToAll(I18N("playerAcquiredWithoutPersuasion", {
                        player = Player[color].steam_name,
                        card = I18N(cardName)
