...
function Helper.getAllCardsInZone(zone)
    local cards = {}
    if not zone then return cards end

    for _, object in pairs(zone.getObjects()) do
        if object.type == "Deck" then
            Helper.addAll(cards, object.getObjects())
...
--- Logs a table or value using JSON encoding.
---@param
function Helper.dumpJson(...)
    local args = table.pack(...)
    for i = 1, args.n do
        local value = args[i]
        if type(value) == "table" then
            Helper._log(JSON.encode(value))
        else
            Helper._log(tostring(value))
        end
    end
end

---@param bag Bag
---@return table cardsAndDecks
function Helper.getCardsAndDecksFromBag(bag)
    local results = {}
    if not bag or bag.tag ~= "Bag" then return results end

    for _, entry in ipairs(bag.getObjects()) do
        table.insert(results, {
            guid = entry.guid,
            nickname = entry.nickname,
            name = entry.nickname or entry.name or "(unnamed)",
            type = entry.type
        })
    end
...
function Helper.dumpHandCards(hand)
    for i, card in ipairs(hand) do
        local tags = card.getTags()
        local tagsStr = ""

        if tags and #tags > 0 then
            tagsStr = table.concat(tags, ", ")
        else
            tagsStr = "none"
        end
...
---Returns only the cards from the list that have the given tag.
---@param cards table
---@param tag string
---@return table
function Helper.filterCardsByTag(cards, tag)
    return Helper.filter(cards, function(card)
        return card.hasTag and card:hasTag(tag)
    end)
end
