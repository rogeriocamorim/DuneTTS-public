...
local PlayBoard = Helper.createClass(nil, {
    ...
    agentParkZoneByColor = {},
    revealParkZoneByColor = {},
    trashGuidPerColor = {
        Red = "ea3fe1",
        Blue = "52a539",
        Green = "4060b5",
...
function PlayBoard:_createAgentCardPark()
    ...
    -- Store agent zone GUID by player color
    if park.zones and #park.zones >= 1 and park.zones[1].getGUID then
        local zoneGUID = park.zones[1]:getGUID()
        PlayBoard.agentParkZoneByColor[self.color] = zoneGUID
    end
...
function PlayBoard:_createRevealCardPark()
    ...
    -- Store reveal zone GUID by player color
    if park.zones and #park.zones >= 1 and park.zones[1].getGUID then
        local guid = park.zones[1]:getGUID()
        PlayBoard.revealParkZoneByColor[self.color] = guid
    end
            ...
            click_function = self:_createExclusiveCallback(function (_, _, altClick)
                if PlayBoard.hasMakerHook(self.color) then
                    ...
                    if PlayBoard.getLeader(self.color).name == "bl_Liet" then
                        Dialog.showYesOrNoDialog(self.color, I18N("lietSandwormConfirm"), continuation, function (confirmed)
                            if confirmed then
...
function PlayBoard:_revealHand(brutal)
    ...
    -- For resource calculation purposes, always use the brutal (right-click) logic
    -- This ensures consistent resource granting regardless of click type
    local properCardForResources = function (card)
        assert(card)
    ...
    -- For actual card movement, use the original logic based on click type
    local properCardForMovement = function (card)
        assert(card)
        if Types.isImperiumCard(card) then
            local choiceOfferingCards = {
                "beneGesseritSister",
    ...
    -- Cards to actually move to reveal park (respects click type)
    local revealedCards = Helper.filter(Player[self.color].getHandObjects(), properCardForMovement)
    
    -- For resource calculation, always use brutal logic for consistency
    local revealedCardsForResources = Helper.filter(Player[self.color].getHandObjects(), properCardForResources)
    local alreadyRevealedCards = Helper.filter(Park.getObjects(self.revealCardPark), properCardForResources)
    local allRevealedCards = Helper.concatTables(revealedCardsForResources, alreadyRevealedCards)
    ...
    -- Calculate contributions from all revealed cards (for persuasion/strength totals)
    local allCardContributions = ImperiumCard.evaluateReveal(self.color, playedCards, allRevealedCards, artillery)
    
    -- Calculate contributions from only newly revealed cards (for resource gains)
    -- Use the consistent brutal-style filtering to prevent duplication
    local newCardContributions = ImperiumCard.evaluateReveal(self.color, playedCards, revealedCardsForResources, artillery)

    self.persuasion:set(
        (allCardContributions.persuasion or 0) +
    ...
    self.strength:set(
        (allCardContributions.strength or 0) +
        ((restrictedOrdnance and councilSeat) and 4 or 0) +
        (swordmasterBonus and 2 or 0))

    --Helper.dump("allCardContributions:", allCardContributions)
    --Helper.dump("newCardContributions:", newCardContributions)

    -- Apply resource effects from ONLY newly revealed cards (prevents double-granting)
    for _, resourceName in ipairs({ "spice", "solari", "water" }) do
        local amount = newCardContributions[resourceName]
        if amount then
            self.leader.resources(self.color, resourceName, amount)
        end
    end

    -- Apply intrigue draws from ONLY newly revealed cards
    local intrigues = newCardContributions.intrigues
    if intrigues then
        self.leader.drawIntrigues(self.color, intrigues)
    end

    -- Apply troop movements from ONLY newly revealed cards (prevents double-granting)
    local createMove = function (category, to)
        return function ()
            local amount = newCardContributions[category] or 0
            if amount > 0 then
    ...
    -- Keep the brutal and not revealed condition only for any special brutal-specific effects
    if brutal and not self.revealed then
        -- Any brutal-specific effects would go here
    end
    ...
    Helper.onceTimeElapsed(0.5).doAfter(function ()
        ...
        if remainingCardToDrawCount > 0 then
            self:_resetDiscard().doAfter(function ()
                self:drawCards(remainingCardToDrawCount, button).doAfter(function (dealOfOtherCardCount)
...
function PlayBoard.grantTechTile(color, techTile)
    -- Log the tech acquisition
    printToAll(I18N("acquireTech", { tech = I18N(Helper.getID(techTile)) }), color)
...
function PlayBoard.giveCard(color, card, isTleilaxuCard)
    ...
    -- Ensure tracking table exists and reset if new round
    local round = TurnControl.getCurrentRound()
    if not PlayBoard.acquiredImperiumCardsByColor or PlayBoard.acquiredImperiumCardsByColor.round ~= round then
        PlayBoard.acquiredImperiumCardsByColor = {
            round = round,
            Red = "",
            Blue = "",
            Green = "",
    ...
    -- Add acquired card name
    local name = ""
    if card then
        name = (card.getName and card.getName()) or (card.nickname or Helper.getID(card))
    end
    local current = PlayBoard.acquiredImperiumCardsByColor[color]
    PlayBoard.acquiredImperiumCardsByColor[color] = current ~= "" and (current .. ", " .. name) or name

    -- Top-deck if conditions met
