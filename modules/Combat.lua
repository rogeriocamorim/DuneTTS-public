...
function Combat.gainObjective(color, objective)
    ...
    -- Check if player has Ornithopter Fleet tech - if so, convert all battle tokens to ornithopter
    local finalObjective = objective
    local finalTag = Helper.toPascalCase(objective, "ObjectiveToken")
    
    if LeaderSelection.hasOrnithopterFleet(color) and objective ~= "ornithopter" then
        -- Convert battle tokens to ornithopter for players with Ornithopter Fleet tech
        finalObjective = "ornithopter"
        finalTag = "OrnithopterObjectiveToken" 
    end
    
    -- Use the final objective type to get the correct position
    local position = PlayBoard.getObjectiveStackPosition(color, finalObjective)

    for _, object in ipairs(Combat.rewardTokenZone.getObjects()) do
        if object.hasTag(finalTag) then
    ...
    local bag = Combat.objectiveTokenBags[finalObjective]
    assert(bag, finalObjective)
    bag.takeObject({
