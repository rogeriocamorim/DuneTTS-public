--[[
    ...
    In case you don't know, the original source code for this mod is available at:
    https://github.com/Chatanga/DuneImmorality/tree/rakis
    But for bloodlines and other stuffs the source code is not available
]]

-- Will be automatically replaced by the build timestamp.
local BUILD = 'TBD'

local MOD_NAME = 'Dune Uprising and Bloodlines'
...
TurnControl = allModules.TurnControl
...
local Controller = {
    ...
    fields = {
        ...
        community = true,
        communityLeaderMode_all = {
            random = "random",
            mode1 = "mode1",
            mode2 = "mode2",
        },
    communityLeaderMode = "random",
...
function asyncOnLoad(scriptState)
    ...
    allModules.ordered = {
        { name = "LocaleModule", module = allModules.Locale },
...
function setLegacy(player, value, id)
    Controller.ui:fromUI(player, value, id)
    if value == "True" then
        Controller.fields.community = false
    else
        Controller.fields.community = XmlUI.DISABLED
    end
    Controller.ui:toUI()
end

--- UI callback (cf. XML).
function setCommunity(player, value, id)
    Controller.ui:fromUI(player, value, id)
    if value == "True" then
        Controller.fields.communityLeaderMode = "random"
    else
        Controller.fields.communityLeaderMode = XmlUI.DISABLED
    end
    Controller.ui:toUI()
end
...
function setImmortality(player, value, id)
    Controller.ui:fromUI(player, value, id)
    if value == "True" then
        Controller.fields.goTo11 = true
        Controller.fields.community = XmlUI.DISABLED
    else
        Controller.fields.goTo11 = XmlUI.DISABLED
        Controller.fields.community = true
    end
    Controller.ui:toUI()
end
...
--- UI callback (cf. XML).
function setCommunityLeaderMode(player, value, id)
    Controller.ui:fromUI(player, value, id)
end
...
function setUpFromUI()
    ...
    setUp({
        ...
        community = Controller.fields.community == true,
    communityLeaderMode = Controller.fields.communityLeaderMode,
        ...
        players = Controller.fields.players,
    })
end
